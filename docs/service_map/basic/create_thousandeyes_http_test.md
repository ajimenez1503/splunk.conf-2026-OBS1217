# Create a ThousandEyes HTTP Server Test with Distributed Tracing

We are going to create an `HTTP Server` test for an endpoint of our OpenTelemetry demo application, which is already instrumented with OpenTelemetry and sending telemetry data to Splunk Observability.
...
- Select `HTTP Server` from the test type options
![Create HTTP Server test](../../img/thousandeyes/createHttpTest1.png)
- Configure Test Settings
      ...
      - Under `HTTP Communication and Performance (Optional)`, check `Enable Distributed Tracing`
